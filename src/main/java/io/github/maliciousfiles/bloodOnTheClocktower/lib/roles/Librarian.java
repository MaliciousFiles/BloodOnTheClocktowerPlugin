...
public class Librarian extends Role {
    ...
    public Librarian(BOTCPlayer me, Game game, RoleInfo info) { super(me, game, info); }
    ...
    public void setup() throws ExecutionException, InterruptedException {
        new StorytellerPauseHook(game.getStoryteller(), "assign the Librarian's Outsider and Wrong reminder tokens").get();

        BOTCPlayer outsider = new PlayerChoiceHook(game, "Outsider reminder token").get();
        newReminderToken(new ReminderToken("Outsider", me, outsider, ReminderToken.Effect.NONE));
    ...
    public void handleNight() throws ExecutionException, InterruptedException {
        ...
        CompletableFuture<Void> instruction = game.getStoryteller().giveInstruction("Select two players for the Librarian" + (me.isImpaired() ? " (impaired)" : ""));
        List<BOTCPlayer> players = new SelectPlayerHook(game.getStoryteller(), game, 2, p->!p.equals(me)).get();
        instruction.complete(null);

        RoleInfo role = new RoleChoiceHook(game.getStoryteller(), game, "Pick a role to show the Librarian" + (me.isImpaired() ? " (impaired)" : ""), 1).get().getFirst();
